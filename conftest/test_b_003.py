...
if month==12 and last_day_tomorrow-1==calendar.monthrange(last_year_today,month)[-1]:
    ...
    last_year_month=1
    last_day_tomorrow=1
    last_year_tomorrow_date=datetime.date(last_year_today,last_year_month,last_day_tomorrow)
    ...
    last_year_tomorrow_date=datetime.date(last_year_today,last_year_month,last_day_tomorrow)
toyear=last_year_today
tomonth=last_year_month
today=last_day_tomorrow
...
def test_a(oa_head):
    ...
    try:
        ...
        if len(response['server-update'][1]['update'])<2:
            ...
            data={
                    "time":"%s_%s_ly"%(last_year_tomorrow_date,oa_head['tohour']),
                    "sale":0.00
                 }
...
def test_b(oa_head):
    ...
    try:
        ...
        if len(response['server-update'][1]['update'])<2:
            ...
            df_money=get_data(response_xx)
            data={
                    "time":"%s_%s_zl"%(last_year_tomorrow_date,oa_head['tohour']),
                    "sale":0.00
                 }
            Way_a().sql_insert(configs['database_config']['period_of_time_sale_table'],data)
            print('没有数据')
